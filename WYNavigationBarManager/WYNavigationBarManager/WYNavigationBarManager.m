...
static const CGFloat kDefaultFullOffset    = 240.f;
static const float   kMaxAlphaValue        = 1.f;
static const float   kMinAlphaValue        = 0.f;
static const float   kDefaultAnimationTime = 0.35f;

#define SCREEN_RECT [UIScreen mainScreen].bounds
#define BACKGROUNDVIEW_FRAME CGRectMake(0 , -20 , SCREEN_RECT.size.width,kNavigationBarHeight)

@interface WYNavigationBarManager()
...
@implementation WYNavigationBarManager

#pragma mark - singlton & private method
+ (WYNavigationBarManager *)sharedManager
{
    static WYNavigationBarManager *manager;
    static dispatch_once_t once_Token;
    dispatch_once(&once_Token, ^{
        manager = [[WYNavigationBarManager alloc] init];
...
+ (void)initBaseData:(WYNavigationBarManager *)manager
{
...
{
    WYNavigationBarManager *manager = [self sharedManager];
...
+ (void)changeAlphaWithCurrentOffset:(CGFloat)currentOffset
{
    ...
    float currentAlpha = [self currentAlphaForOffset:currentOffset];
    ...
    {
        [self changeTintColorWithOffset:currentOffset];
    }
}
...
+ (float)currentAlphaForOffset:(CGFloat)offset
{
    WYNavigationBarManager *manager = [self sharedManager];
